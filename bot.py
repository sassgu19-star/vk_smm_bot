    ...
    for event in longpoll.listen():
        if event.type == VkEventType.MESSAGE_NEW and event.to_me and event.user_id == OWNER_ID:
            ...
            if msg.startswith("/set_time"):
                ...
                save_data(data)
                vk_group_session.method("messages.send", {"user_id": OWNER_ID, "message": "✅ Пост опубликован!", "random_id": 0})
            
            elif msg.lower() == "отклонить" and data.get("pending_post"):
                data["pending_post"] = None
                save_data(data)
                vk_group_session.method("messages.send", {"user_id": OWNER_ID, "message": "❌ Пост отклонён.", "random_id": 0})
...
Когда наступает заданное время, бот берёт тему на сегодня, генерирует пост + картинку, присылает вам на проверку. Вы отвечаете «опубликовать» или «отклонить»."""
                ...
                vk_group_session.method("messages.send", {"user_id": OWNER_ID, "message": "❓ Неизвестная команда. Напишите /help", "random_id": 0})
...
def main():
    ...
    # Запускаем обработчик сообщений (основной поток)
